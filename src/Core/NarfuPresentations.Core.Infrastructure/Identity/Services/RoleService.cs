...
public class RoleService : IRoleService
{
    ...
    public async Task<RoleResponse> GetByIdAsync(string id) =>
        await _dbContext.Roles.SingleOrDefaultAsync(role => role.Id == id)
            is { } role
    ...
    public async Task<string> UpdatePermissionsAsync(UpdateRolePermissionsRequest request, CancellationToken cancellationToken)
    {
        ...
        foreach (var claim in currentClaims.Where(cl => !request.Permissions.Any(p => p == cl.Value)))
        {
    ...
    public async Task<string> DeleteAsync(string id)
    {
        ...
        if ((await _userManager.GetUsersInRoleAsync(role.Name)).Count > 0)
            throw new ConflictException($"Not allowed to delete {role.Name} Role as it is veing used.");
