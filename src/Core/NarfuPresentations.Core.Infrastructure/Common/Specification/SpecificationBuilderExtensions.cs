...
public static class SpecificationBuilderExtensions
{
        ...
        if (filter is not null)
        {
            ...
            if (!string.IsNullOrEmpty(filter.Logic))
            {
                if (filter.Filters is null)
                    throw new ApplicationLayerException("The Filters attribute is required when declaring a logic");
    ...
    private static Expression CreateFilterExpression(
        ...
        string filterOperator) => filterOperator switch
        {
...
#pragma warning restore CA2208 // Правильно создавайте экземпляры исключений аргументов
        ...
        Type propertyType)
    {
        ...
        if (propertyType.IsEnum)
        {
            ...
            if (!Enum.TryParse(propertyType, stringEnum, true, out object? valueparsed))
                throw new ApplicationLayerException(string.Format("Value {0} is not valid for {1}", value, field));
    ...
    private static Filter GetValidFilter(Filter filter)
    {
        if (string.IsNullOrEmpty(filter.Field))
            throw new ApplicationLayerException("The field attribute is required when declaring a filter");
        if (string.IsNullOrEmpty(filter.Operator))
            throw new ApplicationLayerException("The Operator attribute is required when declaring a filter");
        return filter;
    }
