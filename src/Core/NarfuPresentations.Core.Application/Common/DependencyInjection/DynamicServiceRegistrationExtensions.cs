...
public static class DynamicServiceRegistrationExtensions
{
    ...
    public static IServiceCollection AddServices(this IServiceCollection services, Type interfaceType, ServiceLifetime lifetime)
    {
        var serviceDefinitions = AppDomain.CurrentDomain.GetAssemblies()
            ...
            .Select(type => new ServiceDefinition(type.GetInterfaces().FirstOrDefault()!, type, lifetime))
            .Where(serviceDefinition => serviceDefinition.Service is not null && interfaceType.IsAssignableFrom(serviceDefinition.Service));
...
#pragma warning disable CA2208 // Правильно создавайте экземпляры исключений аргументов
    private static IServiceCollection AddService(this IServiceCollection services, ServiceDefinition serviceDefinition) =>
        serviceDefinition.Lifetime switch
        {
            ServiceLifetime.Transient => services.AddTransient(serviceDefinition.Service!, serviceDefinition.Implementation!),
            ServiceLifetime.Scoped => services.AddScoped(serviceDefinition.Service!, serviceDefinition.Implementation!),
            ServiceLifetime.Singleton => services.AddSingleton(serviceDefinition.Service!, serviceDefinition.Implementation!),
            _ => throw new ArgumentException("Invalid Service Lifetime", nameof(ServiceDefinition.Lifetime))
        };
